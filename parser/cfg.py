...
class Symbol(ABC):
    ...
    def __init__(self, name: str, first: Union[Set[str], None], follow: Optional[Set[str]], nullable: Optional[bool]):
        self.name = name
        self.first = first
        self.follow = follow
...
class CFG:
    ...
    def _find_first(self, symbol: Symbol, visited: Optional[Set[Symbol]] = None) -> Set[str]:
        ...
        for rule in self.production_rules:
            if rule.lhs is symbol:
                if isinstance(rule.rhs[0], Terminal) and rule.rhs[0].is_epsilon is False and rule.rhs[0].first is not None:
    ...
    def _find_follow(self, symbol: Symbol, visited: Optional[Set[Symbol]] = None) -> Set[str]:
    ...
    def _generate_parse_table(self) -> None:
        parse_table: Dict[Tuple[NonTerminal, str], Set[ProductionRule]] = defaultdict(set)
        for rule in self.production_rules:
