...
from lexer.basic_symbols import (
    DIGITS,
    NON_ZERO_DIGITS,
    ALPHABET,
    UPPERCASE_ALPHABET,
...
class JsonObject:
    ...
    def ast_to_object(ast: ASTNode) -> object:
        if ast.name == "VALUE":
            return JsonObject.ast_to_object(ast.children[0])
            ...
            if ast.lexme is None:
                raise Exception("Terminals should always have a non null lexme")
            return json.loads(ast.lexme)
        elif ast.name == "ARRAY":
            result = []
            for child in ast.children[1].children:
                if child.name == "ELEMENT":
                    result.append(JsonObject.ast_to_object(child))
            return result
            ...
            for child in ast.children[1].children:
                if child.name == "MEMBER":
                    ...
                    ] = JsonObject.ast_to_object(child.children[2])
            return result
        elif ast.name == "S-Prime":
            return JsonObject.ast_to_object(ast.children[1])
        elif ast.name == "JSON":
            return JsonObject.ast_to_object(ast.children[0])
